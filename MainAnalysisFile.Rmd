...
linear <- gamm4(SleepDurationInSpt ~ fhr + sex + ccMDD + age,
                   random = ~(1 | fid / subject_id / filename),
                   data = merged)


nonlinear <- gamm4(SleepDurationInSpt ~ fhr + sex + ccMDD + s(age, by = fhr, k = 10),
                   random = ~(1 | fid/ subject_id / filename),
                   data = merged)
nonlinear20 <- gamm4(SleepDurationInSpt ~ fhr + sex + ccMDD + s(age, by = fhr, k = 20),
                   random = ~(1 | fid/ subject_id / filename),
                   data = merged)
n10 <- nonlinear10$gam
n20 <- nonlinear20$gam
...
#Adding fid to interept
model3 <- lmer(SRI2_scaled ~ fhr + sex + ccMDD + age + (1| fid /subject_id / filename), data = merged)
anova(model2, model3)


#Comparing a linear to nonlinear model
linear <- gamm4(SRI2_scaled ~ fhr + sex + ccMDD + age,
                   random = ~(1 | fid / subject_id / filename),
                   data = merged)

nonlinear <- gamm4(SRI2_scaled ~ fhr + sex + ccMDD + s(age, by = fhr),
                   random = ~(1 | fid/ subject_id / filename),
                   data = merged)
...
gam_model <- gam(SRI2_scaled ~ fhr + sex + ccMDD +
                   s(age, by = fhr, k = 10) +
                   s(subject_id, bs = "re"),
                 data = merged, method = "REML")
gam.check(gam_model)

diff <- difference_smooths(gam_model, smooth = "s(age)", by = "fhr",
                           comp = list(fhr = c("FHR", "Control")))
draw(diff)
...
pred <- predict(gam_model, newdata = newdat, se.fit = TRUE, exclude = "s(subject_id)")
...
gam_model <- gam(SE ~ fhr + sex + ccMDD + BMI_filled +
                   s(age, by = fhr, k = 25) +
                   s(subject_id, bs = "re"),
                 data = merged, method = "REML")
gam.check(gam_model)
...
age_seq <- seq(min(merged$age), max(merged$age), length.out = 200)
bmi_seq <- seq(
  min(merged$BMI_filled, na.rm = TRUE),
  max(merged$BMI_filled, na.rm = TRUE),
  length.out = 200
)
newdat <- expand.grid(
  age = age_seq,
...
balrgA <- ggplot(newdat, aes(x = age, y = fit, color = fhr)) +
  geom_line(size = 1.2) +
  geom_ribbon(aes(ymin = lower, ymax = upper, fill = fhr), alpha = 0.2, color = NA) +
  scale_y_continuous(limits = c(60,100), breaks = seq(60,100, by = 20)) +
scale_color_manual(values = c("Control" = "#6699FF", "FHR" = "#FF6666")) +
...
###Midpoint
```{r}
gam_model <- gam(sleep_midpoint_min ~ fhr + sex + BMI +
                   s(age, by = fhr, k = 10) +
                   s(subject_id, bs = "re"),
                 data = merged, method = "REML")
gam.check(gam_model)

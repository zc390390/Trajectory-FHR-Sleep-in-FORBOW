...
nonlinear <- gamm4(SleepDurationInSpt ~ fhr + sex + ccMDD + s(age, by = fhr, k = 5),
                   random = ~(1 | fid/ subject_id / filename),
                   data = merged)
...
#Comparing a linear to nonlinear model
linear <- gamm4(SE ~ fhr + sex + ccMDD + age,
                   random = ~(1 | fid / subject_id / filename),
                   data = merged)

nonlinear <- gamm4(SE ~ fhr + sex + ccMDD + s(age, by = fhr),
                   random = ~(1 | fid/ subject_id / filename),
                   data = merged)

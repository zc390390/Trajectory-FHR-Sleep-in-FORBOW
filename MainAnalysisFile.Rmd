...
# Plot 2: Age
p2 <- ggplot(sum, aes (x = age)) + geom_density(fill = "forestgreen", alpha = 0.5, color = "black") +
  scale_x_continuous(limits = c(8,22), breaks = seq(8,22, 2)) +
  labs(title = "Age Distribution", x = "Age", y = "Density") + theme_minimal()
...
ggplot(sum, aes(x = age, y = SleepMidm, color = fhr, fill = fhr)) +
  geom_jitter(alpha = 0.1, width = 0.2, height = 0.5, size = 1.2) +
  geom_smooth(method = "gam", se = FALSE, linetype = "solid", size = 2, aes(group = fhr)) +
  geom_violin(alpha = 0.2, width = 0.5, color = NA, position = position_nudge(x = 0)) +
  scale_color_brewer(palette = "Dark2") +
  scale_fill_brewer(palette = "Dark2") +
...
###Exploratory Analysis - Extreme sleep duration
```{r}
merged <- merged %>%
  mutate(
...
# Plot
ggplot(newdat, aes(x = age, y = fit*100, color = fhr, fill = fhr)) +
  geom_line(size = 1.2) +
...
model3 <- lmer(sleep_midpoint_min ~ fhr*age +  sex + (1| fid/subject_id / filename), data = merged)

model4 <- lmer(sleep_midpoint_min ~ fhr*age +  sex + weekend + (1| fid/subject_id / filename), data = merged)
anova(model3, model4)

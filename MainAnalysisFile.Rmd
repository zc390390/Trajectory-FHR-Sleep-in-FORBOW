...
merged <- Night[FOR, on = .(Subject_ID), allow.cartesian = TRUE]

merged[, diffdays := abs(as.numeric(difftime(Assessment_Date_Night, Assessment_Date_FOR, units = "days")))]

merged <- merged %>% filter(diffdays <= 179)

two <- merged[(Subject_ID %in% c(8000038204, 8000114479))]

```

#Non-Mean Graphing
```{r}
merged$RoundAge <- merged$age
merged$filenamea <- merged$filename
...
d_f_WE <- ggplot(data = merged, mapping = aes( x = age, y = sleeponset, color = group)) +
  geom_jitter(alpha = 0.3, width = 0.2, height = 0.5, size = 1.5) +
  geom_smooth(method = "gam", aes(group = group), se = FALSE, linetype = "solid", size = 1.5) +
  scale_color_brewer(palette = "Dark2") + 
  scale_y_continuous(
    limits = c(20, 28),
    breaks = seq(20, 28, by = 2)

...
<<<<<<< HEAD:RMD Files/TST Analysis.Rmd

linear <- lmer (SleepDurationInSpt ~ fhr*age + sex + age*weekend + (1|fid/subject_id), data = merged)
summary(linear)
=======
library(mgcv)
>>>>>>> a14b72a8c6c3eeb5682c42f968e52441544898cc:TST Analysis.Rmd

merged2 <- merged[!is.na(merged$BMI_filled), ]
linear <- gamm4(SleepDurationInSpt ~ fhr*age + sex,
                   random = ~(1 | fid / subject_id / filename), REML=FALSE,
                   data = merged)
summary(linear)
merged$age2 <- (merged$age)^2
linear <- lmer(SleepDurationInSpt ~ fhr*age + sex +
                    (1 | fid/subject_id/filename), data = merged, REML = FALSE)
linear_poly <- lmer(SleepDurationInSpt ~ fhr*age + fhr*age2 + sex +
                    (1 | fid/subject_id/filename), data = merged, REML = FALSE)
anova(linear, linear_poly)
summary(linear_poly)

#Midpoint, SRI, TST
pvals <- c(0.00552, 0.0144, 0.0229)

p.adjust(pvals, method = "BH")


nonlinear <- gamm4(SleepDurationInSpt ~ fhr + sex + s(age, by = fhr, k = 5) + s(age, k = 5),
                   random = ~(1 | fid/ subject_id / filename), REML=FALSE,
                   data = merged)
...
=======
gam(SleepDurationInSpt ~ sex + fhr + s(age, by = fhr, k = 5),
    data = merged)
>>>>>>> a14b72a8c6c3eeb5682c42f968e52441544898cc:TST Analysis.Rmd
```

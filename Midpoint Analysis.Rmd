...
merged <- merged %>%
  mutate(
    sleeponset = as.numeric(sleeponset),
    SptDuration = as.numeric(SptDuration),
    midpoint = (sleeponset + (SptDuration / 2)) %% 1440
  )
...
sum <- merged %>%
  group_by(filename) %>%
  summarise(midpointm = mean((midpoint), na.rm=TRUE),
...
p4 <- ggplot(sum, aes(x = SleepMidm - 24)) +
...
merged$midpointadj1 <- merged$midpoint
merged$midpointadj2 <- (merged$sleep_midpoint_min/60) + 24
merged$wakeupadj <- merged$wakeup

summary(merged$midpoint)
summary(merged$sleep_midpoint_min)

merged$sleep_mid2_adj <- (merged$sleep_midpoint_min/60) + 12
summary(merged$sleep_mid2)


ggplot(merged, aes(x = age, y = sleep_mid2_adj, color = fhr)) +
...
longdat <- merged %>%
  pivot_longer(
    cols = c(sleeponsetadj, midpointadj1, midpointadj2, wakeupadj),
...
my_colors <- c(
  "Control.sleeponsetadj" = "#4C72B0",
  "Control.midpointadj"   = "#55A868",
  "Control.wakeupadj"     = "#8172B2",
  
  # FHR reds - same hue, different lightness
  "FHR.sleeponsetadj" = "#B22222",  # firebrick
  "FHR.midpointadj"   = "#D33F49",  # classic red
  "FHR.wakeupadj"     = "#E57373"   # soft rose
)
...
sum <- merged %>%
  group_by(filename) %>%
  summarise(midpointadj = mean((sleep_mid2_adj), na.rm=TRUE),
...
merged$sleep_midpoint_min <- merged$midpoint - 24
summary(merged$sleep_midpoint_min)
